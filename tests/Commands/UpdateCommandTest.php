...
CONTENT;
    ...
    #[Test]
    public function updatesExistingGitattributesWithNegatedExportIgnores(): void
    {
        if ((new OsHelper())->isWindows()) {
            $this->markTestSkipped('Skipping test on Windows systems');
        }
...
bin/old -export-ignore
src/old -export-ignore
CONTENT;
...
CONTENT;
        ...
        $this->assertSame(
            \rtrim($expectedGitattributesContent),
            \rtrim((string) \file_get_contents($this->temporaryDirectory . DIRECTORY_SEPARATOR . '.gitattributes'))
        );
    }
    ...
    #[Test]
    public function printsExpectedNegatedContentWithoutWritingAFile(): void
    {
        $this->createTemporaryFiles(
            ['.gitignore', 'composer.json'],
            ['bin', 'src']
...
CONTENT;
        ...
        $this->assertSame(\trim($expectedOutput), \trim($result->output()));
